...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        all_fc1_params = torch.cat([x.view(-1) for x in net.fc1.parameters()])
        all_fc2_params = torch.cat([x.view(-1) for x in net.fc2.parameters()])
        all_fc3_params = torch.cat([x.view(-1) for x in net.fc3.parameters()])
